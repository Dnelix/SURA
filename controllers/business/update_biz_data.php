...
try {
    ...
    $query = $readDB -> prepare ('SELECT id FROM tbl_business WHERE owner = :userid');            
    ...
    $queryString = "UPDATE tbl_business SET ".$queryFields." WHERE owner = :userid";
    ...
    $query = $writeDB -> prepare('SELECT '. $table_columns .' FROM tbl_business WHERE owner = :userid');
    $query -> bindParam(':userid', $userid, PDO::PARAM_INT);
    $query -> execute();
    ...
    sendResponse(200, true, "Business Record Updated", $returnData);
    exit();
